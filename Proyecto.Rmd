...
# Vector con los días de la semana en el orden deseado
dias <- c("lunes", "martes", "miércoles", "jueves", "viernes", "sábado", "domingo")


# Convertir a factor con el orden deseado
dias_semana <- factor(dias_semana, levels = dias)

# Contar el número de ocurrencias de cada día
ocurrencias <- table(dias_semana)

# Calcular el porcentaje de ocurrencias de cada día
porcentaje <- prop.table(ocurrencias) * 100
...
ggplot(datos_grafico, aes(x = reorder(dias, ocurrencias), y = ocurrencias)) +
  geom_bar(stat = "identity", fill = "skyblue") +
  geom_text(aes(label = paste(round(porcentaje, 1), "%")), vjust = -0.5, color = "black") +
  labs(title = "Número de ocurrencias por día de la semana",
       x = "Día de la semana",
...
Los días que más se ha comprado son tanto martes como sábado, con un 23.5% de los tickets para cada día.

6. ¿Existe alguna relación entre la cantidad de productos de la compra y la hora a la que se ha hecho?

7. ¿En qué Mercadona se han comprado más productos?
