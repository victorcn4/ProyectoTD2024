...
author: "Laura Beltran Pellicer, Victor Cardama Noguera, Mara Ferrús Castells," 
"Jorge García Pozuelo, Sandra López Llorens""
date: "2024-05-14"
...
Unimos todos los tickets en un mismo data frame.
```{r}
#Listamos todos los archivos de tickets en la carpeta 'data'.
lista_archivos <- list.files(path = "data", full.names = TRUE)

#Creamos una lista para almacenar todos los tickets.
lista_tickets <- list()

#Verificamos las columnas de cada archivo y almacenamos solo los que tienen la misma estructura.
columnas_iguales <- NULL
for (archivo in lista_archivos) {
  ticket <- read.csv(archivo)
  if (is.null(columnas_iguales)) {
    columnas_iguales <- colnames(ticket)
  } else {
    if (identical(colnames(ticket), columnas_iguales)) {
      lista_tickets[[archivo]] <- ticket
    } else {
      print(paste("El archivo", archivo, "no tiene las mismas columnas. No se agregará a la lista."))
    }
  }
}

#Unimos todos los tickets en un único dataframe.
if(length(lista_tickets) > 0) {
  all_tickets <- do.call(rbind, lista_tickets)
} else {
  print("No hay archivos con la misma estructura de columnas.")
}
```
...
la dirección, fecha, hora de la compra, ... que la cantidad comprada de un 
producto determinado.

Para cada una de las columnas de nuestro data frame en bruto, aplicamos una serie
de condiciones que nos permitirán "limpiar" los elementos no interesantes de nuestro
análisis.

Por ejemplo, si algunos productos tienen la cantidad adquirida delante, sabemos 
que el número delante del str será la cantidad adquirida y podemos almacenarla 
en otra columna, a la vez que la borramos de la variable de productos.

Otro ejemplo es con la fecha y la hora.
Al leerla en una linea nos queda un formato de esta manera: "d/m/Y XX:XX".
Si queremos almacenarlas en variables diferentes, podemos poner una condicion de
que los elementos de la columna fecha tengan esta estructura "d/m/Y" y los 
elementos que tengan estructura tipo "XX:XX" los podemos almacenar en otra 
variable.
...
Se debería hacer una comprobación para ver si hay alguna observación con valores 
faltantes para su posterior depuración, ya sea eliminando esa observación, o 
imputando otro tipo de dato según una serie de criterios.
...
está hecho de forma eficiente, tendríamos que sacar toda la información sobre 
nuestra compra en este data frame.
