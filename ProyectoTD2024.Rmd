...
Observamos que en ningún ticket se encuentra algún "dato faltante". Esto es debido a que no todos los tickets presentan los mismos productos, pero en sí si que siguen una estructura en concreta la cual todos los tickets deben seguir para un procesamiento correcto.
...
```{r,echo=FALSE,warning=FALSE,message=FALSE, fig.cap='Figura2 : Outliers'}
...
# Convertir importe_total a tipo numérico
tickets_juntos$importe_total <- as.numeric(tickets_juntos$importe_total)

# Crear el boxplot de los importes totales
boxplot(tickets_juntos$importe_total, 
        main = "Boxplot de Importes Totales",
        ylab = "Importe Total")
...
producto podemos observar varios outliers muy por encima de los percentiles. 
...
```{r Figura3, message=FALSE, warning=FALSE, echo = FALSE, fig.cap="Figura 3: Frecuencia por producto"}
...
En \@ref(fig:Figura3) vemos como el producto más comprado, con diferencia, es el "queso lonchas cabra", seguido por "brotes tiernos maxi" y "queso fresco cabra" con la mitad de apariciones. Previamente se ha filtrado los productos para únicamente mostrar los productos que han sido comprados más de dos veces.
...
```{r,echo=FALSE, fig.cap='Figura 3: Ocurrencias por semana'}
...
Esto puede deberse a que los miércoles, como se trata de mitad de semana, los clientes van a comprar más frecuentemente para "llegar" a final de semana, y los sábados es el dia de la semana no laborable con muchos supermercados abiertos, por tanto la gente puede ir a comprar con más frecuencia.
...
```{r,echo=FALSE,warning=FALSE, fig.cap='Figura 4: Nº Compras'}
...
En esta matriz de gráficos tenemos, en la parte izquierda un gráfico donde se muestra el número de compras que se han hecho en cada mercadona. Podemos observar claramente como el del C.C AQUA es en el que más compras se hacen. Teniendo esto en cuenta, la empresa debería tomar medidas como reforzar el personal o agrandar los pedidos. Por lo contrario vemos como tanto el supermercado de la Calle Periodista Azzati, como el de la Calle Valencia son en los que menos se compra.

Por otra parte, en el gráfico de la derecha observamos el número de productos comprados en cada supermercado. Observamos como el del C.C AQUA es en el que más productos se compran lo que reafirma que se deberían mejorar los servicios en dicho establecimiento. 

## Análisis multivariante.

Ahora vamos a incidir en un análisis de las medias de cada variable establecida en la tabla:

```{r Figura5, message=FALSE, warning=FALSE, echo = FALSE, fig.cap="Figura 5: Medias"}
library(ggplot2)
...
En \@ref(fig: Figura5) vemos como el Q-Q plot establece que la distribución de la media de los importes de compra en Mercadona está generalmente cerca de una distribución normal, pero podría haber algunas desviaciones en ciertos rangos de valores.
